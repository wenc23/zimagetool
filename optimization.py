...
from utils import print_section, print_success, print_error, get_integer_input


def select_optimization_mode():
    """让用户选择优化模式"""
    print_section("🔧 请选择显存优化模式", width=50)
    ...
    while True:
        ...
        if choice == 1:
            print_success("已选择: 基础优化模式")
            return "basic"
        elif choice == 2:
            print_success("已选择: 低显存优化模式")
            return "low_vram"
...
def apply_low_vram_optimizations(pipe):
    ...
    try:
        ...
        print_success("低显存优化已启用")
    except Exception as e:
        print_error(f"启用低显存优化时出错: {e}")
        print("💡 尝试使用基本优化...")
        # 如果出错，至少启用注意力切片
        pipe.enable_attention_slicing("max")
        print_success("已启用基本优化")
